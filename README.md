...
> *Code generated with AI assistance, inspiration provided by humans. If it works, ship it!*
...
1. **Vibe Coding Product**: Most of this code was generated by LLMs (Claude/Gemini) under my "mind control". The code style may not be rigorous, the logic may be wild, but it solves my problem.
2. **Breaking Risk**: This extension relies on parsing Gemini's web page DOM structure. **If Google updates the Gemini interface, the extension will break immediately.** You'll need to F12 and fix it yourself with AI's help.
3. **Maintenance**: **NO guarantee** of long-term maintenance. I'm sharing this for developers who want to learn or tinker, not as a commercial product.
4. **Data Privacy**: This is a pure client-side extension. Data only flows between **your browser**, **Google Gemini API**, and **your local Obsidian**. It is **NEVER** uploaded to any third-party server.
...
- **AI Knowledge Graph**: **Standalone module**, generates Canvas mind maps on-demand via Gemini API
- **Smart Filtering**: Filters out chit-chat, extracts core knowledge points
- **Visual Enhancements**: Hub & Spoke layout, semantic coloring, bidirectional links
- **Multi-language**: Choose the language for AI-generated content (English/Chinese)
...
2. **⚠️ Critical Step: Load the Full Conversation**
   ...
   - *Skipping this step will result in incomplete exports*

3. Click the **Horizon Bridge** icon in your browser toolbar to open the panel:

   **Phase 1: Basic Sync**
   - **Check Status**: Click **"Check Sync Status"** to verify connection to Obsidian
   - **Export Files**: Click **"Sync to Obsidian"**
     - *This saves conversation as Markdown files (`001-xxx.md`, `002-xxx.md`...)*

   **Phase 2: AI Enhancement (Optional)**
   - **Select Language**: Choose your preferred language for AI-generated content
   - **Generate Index**: Click **"Generate _INDEX.md"**
     - *Creates a homepage with navigation and summary*
   - **Generate Graph**: Click **"Generate Logic Graph (AI)"**
     - *This consumes Gemini API quota, may take 30-60 seconds for long conversations*

4. Check results in Obsidian under `Gemini/` folder
...
**Q: Why separate "Sync" and "Generate Graph" into two buttons?**

For flexibility and token savings. Sometimes you just want to backup conversations without spending AI quota on graph generation. Separating them also prevents file save failures due to AI timeouts.
...
This project is a **Vibe Coding** product.

If you find bugs or want new features:
1. **Recommended**: Ask AI (Claude/Gemini/GPT), give it the code, describe your needs, and fix it yourself
2. **Pull Request**: If you've made a good fix, PRs are welcome
